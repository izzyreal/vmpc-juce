...
namespace vmpc_juce::gui::vector
{
    ...
    public:
        ...
        void paint(juce::Graphics &g) override
        {
            ...
            {
                float line_angle =
                    juce::MathConstants<float>::twoPi * (i / 12.0f + angle);
                float brightness =
                    0.7f + 0.5f * std::cos(line_angle +
                                           juce::MathConstants<float>::pi);
                brightness = std::min<float>(brightness, 0.5f);
                juce::Colour greyColor =
                    juce::Colours::grey.withBrightness(brightness);
                juce::Colour blackColor = juce::Colours::black;

                juce::Point<float> direction(std::sin(line_angle),
                                             -std::cos(line_angle));
                juce::Point<float> outerPoint =
                    center + direction * innerRadius;
                juce::Point<float> innerPoint =
                    outerPoint - direction * lineLength;
                juce::Point<float> offset(-direction.y * lineOffset,
                                          direction.x * lineOffset);

                g.setColour(blackColor);
                g.drawLine(innerPoint.x - offset.x, innerPoint.y - offset.y,
                           outerPoint.x - offset.x, outerPoint.y - offset.y,
                           line_thickness);
