...
namespace vmpc_juce::gui::vector
{
    ...
    {
    public:
        KeyTooltip(const std::function<std::string()> &getTooltipTextToUse,
            ...
            : getTooltipText(getTooltipTextToUse),
              positionAnchor(positionAnchorToUse), getFont(getFontToUse),
              getScale(getScaleToUse), hardwareLabel(hardwareLabelToUse),
                                     ...
                                     bool /* wasResized */) override
        {
            const float scaleFactor =
                std::log(getScale() + 2.0f) / std::log(2.0f);

            const auto translateX =
                (unscaledOffsetFromAnchor.first - 2.5f) * scaleFactor;
            const auto translateY =
                (unscaledOffsetFromAnchor.second + 1.5f) * scaleFactor;

            auto targetBounds =
                positionAnchor->getLocalBounds().toFloat().translated(
                    translateX, translateY);
            auto targetCenter =
                positionAnchor->localPointToGlobal(targetBounds.getCentre());
            ...
            {
                targetCenter =
                    tooltipParent->getLocalPoint(nullptr, targetCenter);
            }

            const auto tooltipTypeScaleFactor =
                shouldMimicPhysicalKeyRepresentation() ? 1.1f : 1.f;
            ...
            setBounds(newX, newY, static_cast<int>(newWidth),
        ...
        void paint(juce::Graphics &g) override
        {
            ...
            const bool mimicPhysicalKeyRepresentationEnabled =
                shouldMimicPhysicalKeyRepresentation();

            auto fontHeight =
                (static_cast<float>(getHeight()) * 1.2f -
                 ((bidirectionalMarginBetweenTextAndBorder + lineThickness) *
                  2)) -
                (shadowSize * 2);
            ...
            const std::string string1 = mimicPhysicalKeyRepresentationEnabled
                                            ? tooltipText.substr(0, 1)
                                            : tooltipText;
            const std::string string2 = mimicPhysicalKeyRepresentationEnabled
                                            ? tooltipText.substr(1, 1)
                                            : "";

            auto textWidth = g.getCurrentFont().getStringWidthFloat(string1);

            const float radius = 3.f;
            const auto totalWidthWithoutText =
                ((lineThickness + horizontalMarginBetweenTextAndBorder +
                  bidirectionalMarginBetweenTextAndBorder) *
                 2.f) -
                (shadowSize * 2);

            const auto totalHeight =
                (g.getCurrentFont().getHeight() *
                 (mimicPhysicalKeyRepresentationEnabled ? 1.3f : 1.f)) +
                (lineThickness + bidirectionalMarginBetweenTextAndBorder) * 2;

            if (totalWidthWithoutText + textWidth + (shadowSize * 2) >
                static_cast<float>(getWidth()))
            {
                textWidth =
                    (static_cast<float>(getWidth()) - totalWidthWithoutText) -
                    (shadowSize * 2);
                textWidth -= 5.f;
            }

            auto totalWidth = mimicPhysicalKeyRepresentationEnabled
                                  ? totalHeight
            ...
            auto outer_rect = juce::Rectangle<float>(
                (static_cast<float>(getWidth()) -
                 (textWidth + totalWidthWithoutText)) /
                    2,
                (static_cast<float>(getHeight()) - totalHeight) / 2, totalWidth,
                totalHeight);
            ...
            if (mimicPhysicalKeyRepresentationEnabled)
            {
                g.drawText(string1, outer_rect.translated(0.f, scale * 2.1f),
                           juce::Justification::centred);
                g.drawText(string2, outer_rect.translated(0.f, -(scale * 4.6f)),
                           juce::Justification::centred);
            }
            else
            {
                g.drawText(string1, outer_rect.translated(0.f, -(scale * 1.3f)),
                           juce::Justification::centred);
            }
        }
