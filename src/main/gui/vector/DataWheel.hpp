...
namespace vmpc_juce::gui::vector
{
    ...
    public:
        ...
        {
            backgroundSvg =
                new SvgComponent({"data_wheel_without_dimple_and_lines.svg"},
                                 commonParentWithShadow, shadowSize, getScale);
            addAndMakeVisible(backgroundSvg);

            lines = new DataWheelLines(getScale);
            addAndMakeVisible(lines);

            dimpleSvg = new SvgComponent({"data_wheel_dimple.svg"},
                                         commonParentWithShadow, 0.f, getScale);
        ...
        void sharedTimerCallback() override
        {
            ...
            const float animationSpeedToUse =
                std::abs(diff) > 0.02f ? animationSpeed : 100.f;
            // Smoothly move displayAngle toward targetAngle
            const float step =
                diff * juce::jmin(animationSpeedToUse * 0.016f, 1.0f);
            displayAngle += step;
    ...
    private:
        ...
        void handleAngleChanged(const float newAngle)
        {
            ...
            const auto radius = std::min(static_cast<float>(getWidth()),
            ...
            const auto yPos =
                centerY - std::cos(theta) * radius - height / 2.0f;

            dimpleSvg->setBounds(static_cast<int>(xPos), static_cast<int>(yPos),
