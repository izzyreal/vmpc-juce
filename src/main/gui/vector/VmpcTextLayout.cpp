...
namespace vmpc_juce::gui::vector
{
    ...
    static juce::String substring(const juce::String &text,
                                  juce::Range<int> range)
    {
        return text.substring(range.getStart(), range.getEnd());
    }

    VmpcTextLayout::Glyph::Glyph(int glyph, juce::Point<float> anch,
                                 float w) noexcept
    ...
    //==============================================================================
    VmpcTextLayout::Line::Line(juce::Range<int> range, juce::Point<float> o,
                               float asc, float desc, float lead,
                               int numRunsToPreallocate)
        : stringRange(range), lineOrigin(o), ascent(asc), descent(desc),
          leading(lead)
    {
    ...
    {
        return {lineOrigin.y - ascent, lineOrigin.y + descent};
    }
    ...
    VmpcTextLayout::VmpcTextLayout(VmpcTextLayout &&other) noexcept
        : lines(std::move(other.lines)), width(other.width),
          height(other.height), justification(other.justification)
    {
    }
    ...
    void VmpcTextLayout::draw(juce::Graphics &g,
                              juce::Rectangle<float> area) const
    {
        auto origin = justification
                          .appliedToRectangle(
                              juce::Rectangle<float>(width, getHeight()), area)
                          .getPosition();
        ...
        for (auto &line : *this)
        {
            ...
            for (auto *run : line.runs)
            {
                ...
                {
                    context.drawGlyph(glyph.glyphCode,
                                      juce::AffineTransform::translation(
                                          lineOrigin.x + glyph.anchor.x,
    ...
    void VmpcTextLayout::createLayout(const juce::AttributedString &text,
                                      float maxWidth)
    {
    ...
    void VmpcTextLayout::createLayoutWithBalancedLineLengths(
        const juce::AttributedString &text, float maxWidth)
    {
        createLayoutWithBalancedLineLengths(text, maxWidth, 1.0e7f);
    }

    void VmpcTextLayout::createLayoutWithBalancedLineLengths(
        const juce::AttributedString &text, float maxWidth, float maxHeight)
    {
        ...
        while (maxWidth > minimumWidth)
        {
            ...
            auto line1 = lines.getUnchecked(lines.size() - 1)
                             ->getLineBoundsX()
                             .getLength();
            auto line2 = lines.getUnchecked(lines.size() - 2)
                             ->getLineBoundsX()
                             .getLength();
    ...
    namespace VmpcTextLayoutHelpers
    {
        struct Token
        {
            Token(const juce::String &t, const juce::Font &f, juce::Colour c,
                  bool whitespace)
        ...
        struct TokenList
        {
                              ...
                              VmpcTextLayout &layout)
            {
                ...
                for (int i = 0; i < tokens.size(); ++i)
                {
                    ...
                    t.font.getGlyphPositions(
                        getTrimmedEndIfNotAllWhitespace(t.text), newGlyphs,
                        xOffsets);
                    ...
                    {
                        currentLine = std::make_unique<VmpcTextLayout::Line>();
                    }
                    ...
                    if (numGlyphs > 0)
                    {
                        currentRun->glyphs.ensureStorageAllocated(
                            currentRun->glyphs.size() + newGlyphs.size());
                        auto tokenOrigin = t.area.getPosition().translated(
                            0, t.font.getAscent());
                        ...
                        auto glyphOffset =
                            tokenOrigin - currentLine->lineOrigin;

                        for (int j = 0; j < newGlyphs.size(); ++j)
                        {
                            auto x = xOffsets.getUnchecked(j);
                            currentRun->glyphs.add(VmpcTextLayout::Glyph(
                    ...
                    {
                        if (t.font != nextToken->font ||
                            t.colour != nextToken->colour)
                        {
                            addRun(*currentLine, currentRun.release(), t,
                        ...
                        if (t.line != nextToken->line)
                        {
                            ...
                            {
                                currentRun =
                                    std::make_unique<VmpcTextLayout::Run>();
                            }

                            addRun(*currentLine, currentRun.release(), t,
                                   runStartPosition, charPosition);
                            currentLine->stringRange = {lineStartPosition,
                                                        charPosition};
                ...
                if ((text.getJustification().getFlags() &
                     (juce::Justification::right |
                      juce::Justification::horizontallyCentred)) != 0)
                {
                    auto totalW = layout.getWidth();
                    bool isCentred =
                        (text.getJustification().getFlags() &
                         juce::Justification::horizontallyCentred) != 0;
        ...
        private:
            static void addRun(VmpcTextLayout::Line &glyphLine,
                               VmpcTextLayout::Run *glyphRun, const Token &t,
                               int start, int end)
            {
                ...
                glyphLine.ascent =
                    juce::jmax(glyphLine.ascent, t.font.getAscent());
                glyphLine.descent =
                    juce::jmax(glyphLine.descent, t.font.getDescent());
            ...
            void appendText(const juce::String &stringText,
            ...
            void layoutRuns(float maxWidth, float extraLineSpacing,
                            juce::AttributedString::WordWrap wordWrap)
            {
                ...
                for (i = 0; i < tokens.size(); ++i)
                {
                    ...
                    if (t.isNewLine ||
                        ((!nextTok->isWhitespace) &&
                         (tokenTooLarge &&
                          wordWrap != juce::AttributedString::none)))
                    {
            ...
            void addTextRuns(const juce::AttributedString &text)
            {
                ...
                for (int i = 0; i < numAttributes; ++i)
                {
                    ...
                    appendText(substring(text.getText(), attr.range), attr.font,
                               attr.colour);
                }
            }

            static juce::String
            getTrimmedEndIfNotAllWhitespace(const juce::String &s)
            {
    ...
    void
    VmpcTextLayout::createStandardLayout(const juce::AttributedString &text)
    {
