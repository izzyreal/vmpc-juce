...
namespace vmpc_juce::gui::vector
{
    ...
    {
    public:
        SimpleLabel(const std::function<float()> &getScaleToUse,
                    const std::string textToUse, const juce::Colour colourToUse,
        ...
        void paint(juce::Graphics &g) override
        {
            ...
            g.setColour(colour);
            const auto yOffset =
                !nearlyEqual(topMargin, -1.f)
                    ? topMargin
                    : std::floor((static_cast<float>(getHeight()) -
                                  getRequiredHeight()) /
                                 2.f);
        ...
        float getRequiredHeight() override
        {
            const auto newlineCount =
                (float)std::count(text.begin(), text.end(), '\n');
            const auto labelHeight =
                ((Constants::BASE_FONT_SIZE * (newlineCount + 1)) +
